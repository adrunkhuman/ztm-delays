...
No marker is expected for failed runs. Workspaces are removed after pending metadata or failure by default; set `MATCHER_SHADOW_KEEP_WORKSPACE=true` only for an explicitly supervised investigation. Prior shadow tables and markers remain intact. A changed artifact under an already committed run ID gets a new table identity, then is rejected before load when the existing marker's immutable content differs. A concurrent marker conflict after that check can leave uncommitted content-addressed tables; inspect them and delete only tables unreferenced by retained marker or pending metadata. With `MATCHER_SHADOW_STRICT=false` load or comparison failure is logged and canonical work can still complete; set strict mode only for an explicitly supervised validation window. Non-strict mode commits a marker even when the gate warns or fails. Strict mode rejects only hard structural/resource gate failures before marker creation. Differences and delay aggregates are evidence, not cutover acceptance. Lineage or duplicate-grain failures prevent the marker in either mode. Defaults cap input at 5,000 GPS objects/20 GiB, comparison scans at 5 GiB, marker payloads at 20 MiB, and retain a 5 GiB free-disk reserve before download; increase them only with a measured budget.

For a historical correction investigation, run the standalone `matcher_historical_correction.py` planner manually with explicit `--start-date` and `--end-date`. It is not scheduled and neither downloads nor loads data. It rejects ranges before `WAREHOUSE_HISTORY_START_DATE`, ranges over 31 days unless explicitly configured, degraded 2026-07-05 through 2026-07-07 dates, and missing snapshot or GPS inputs. Review its exact snapshot mappings, GCS generations/hashes/bytes, affected current/prior partitions, and BigQuery dry-run/cap command before authorizing any separate correction work.
