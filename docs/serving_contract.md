...
| `mart_pipeline_status` | Operational `service_date`, `mode` | Recent-days status table | `service_date`, `mode`, `completeness_ratio`, `service_coverage_ratio`, `trips_complete`, `trips_partial`, `trips_broken`, `stop_arrivals_count`, `latest_gtfs_snapshot_at`, `health_ratio`, `health_label`, `status_rank_desc` |
