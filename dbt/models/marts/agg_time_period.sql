...
{{
    config(
        ...
        cluster_by=["mode", "hour_bracket", "line"],
...
period_rows as (
    select
        service_date,
        gtfs_snapshot_id,
        -- Month time rows are network-wide within mode; schedule-version rows keep line scope.
        cast(null as string) as line,
        cast(null as string) as route_short_name,
        mode,
        cast(null as int64) as direction_id,
        cast(null as string) as trip_headsign,
        hour_bracket,
        schedule_day_type,
        cast(null as string) as schedule_version_id,
        delay_seconds,
        day_type,
        weekday_name,
        'month' as period_type,
