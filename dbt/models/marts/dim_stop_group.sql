...
group_snapshots as (
    select
        ...
        string_agg(distinct posts.zone_id, ' | ' order by posts.zone_id) as zone_ids,
        string_agg(distinct posts.effective_zone_id, ' | ' order by posts.effective_zone_id) as effective_zone_ids,
        string_agg(distinct posts.stop_name_stem, ' | ' order by posts.stop_name_stem) as stop_name_stems,
        string_agg(distinct posts.town_name, ' | ' order by posts.town_name) as town_names,
...
group_states as (
    select
        ...
        group_snapshots.zone_ids,
        group_snapshots.effective_zone_ids,
        group_snapshots.stop_name_stems,
        group_snapshots.town_names,
