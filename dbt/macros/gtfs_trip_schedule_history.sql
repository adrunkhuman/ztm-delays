{% macro gtfs_trip_schedule_history(mapping_sql, snapshot_ids=none) %}
with processing_snapshots as ({{ mapping_sql }}),
{% for name in ['calendar_dates', 'trips', 'stop_times'] %}
bounded_{{ name }} as (
    select * from {{ ref('stg_gtfs__' ~ name) }}
    {% if snapshot_ids is not none %}
    where gtfs_snapshot_id in ({{ schedule_snapshot_literals(snapshot_ids) }})
    {% endif %}
),
{% endfor %}
processing_service_dates as (
    select
        processing_date,
        processing_date as service_date,
        gtfs_snapshot_id
    ...
    select
        processing_date,
        date_sub(processing_date, interval 1 day) as service_date,
        gtfs_snapshot_id
    from processing_snapshots
),
...
calendar_dates as (
    ...
    inner join bounded_calendar_dates as calendar_dates
        on processing_service_dates.service_date = calendar_dates.service_date
        and processing_service_dates.gtfs_snapshot_id = calendar_dates.gtfs_snapshot_id
),
...
trip_stop_times as (
    ...
    from active_trips
    inner join bounded_stop_times as stop_times
